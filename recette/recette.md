...
- **Feature R — AI transport extended (AIZ_ zones + vehicle whole-unit)** : 26 fonctionnels = **71 cas** ✅ PASS (F-R-1→F-R-26 71/71 [2026-05-19]) — `_parseAIZ` : format 3/5 champs, coalition A/R/B/N, rôles P/D, stock, aiDropMode G/P/GP, backward compat, erreurs ; `_discoverAIZ` : scan zones ME + création CTLDTroopZone avec isAIPickup/isAIDropoff/aiDropMode ; `_loadLegacyZones` : AIZones config avec rôle P/D + stock/aiDropMode ; `getAIPickupZoneAt`/`getAIDropoffZoneAt` : filtre coalition + lat/long ; `_validateZoneNames` : chevauchement P+D même coalition WARN ; `getTroopZoneAtPoint` : guard isAIPickup exclu menu joueur ; `_checkAIStatus` : ordre dropoff→vehicle pickup→troop pickup (Option B) ; `cleanupDeadTransports` câblé S_EVENT_DEAD ; aiDropMode G/P/GP default ; fallback scan templates (F-R-21→F-R-26) : premier compatible sélectionné, scan circulaire random, hasTroops guard, teams vide guard
