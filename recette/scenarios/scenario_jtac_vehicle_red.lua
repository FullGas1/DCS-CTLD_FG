...
-- Injectable scenario: RED JTAC ground vehicle (SKP-11) vs BLUE enemy (Sol_g-1 / LAV-25)
-- Mirrors the flow of CTLDCrateManager:_spawnUnpacked → _dispatchPostSpawn → startLase
--
-- Pre-requisites:
--   - BLUE group "Sol_g-1" must exist (use diag/diag_spawn_sol_g1.lua to restore if needed)
--   - Config param: JTAC_dropEnabled = true (default)
--
-- Sequence:
--   T+0s  : (0) cleanup existing ground JTACs
--            (1) spawn SKP-11 100m from Sol_g-1 → startLase()
--   T+15s : VERIFY 1 — JTAC lasing Sol_g-1-1 (currentTarget set)
--   T+45s : (2) destroy Sol_g-1 (guaranteed target loss)
...
local VEHICLE_COA    = coalition.side.RED
local VEHICLE_CTY    = country.id.RUSSIA

-- ── helpers ──────────────────────────────────────────────────────────────────

local function report(msg)
    trigger.action.outText("[JTAC-VEH-RED] " .. msg, 25)
    ctld.utils.log("INFO", "[scenario_jtac_vehicle_red] " .. msg)
end
...
if not enemyGrp or not enemyGrp:isExist() then
    return "ABORT: enemy group not found: " .. ENEMY_GROUP .. " — run diag_spawn_sol_g1.lua first"
end
...
return string.format("scenario_jtac_vehicle_red started | '%s' | gname='%s' | enemy '%s' @ (%.0f,%.0f)",
