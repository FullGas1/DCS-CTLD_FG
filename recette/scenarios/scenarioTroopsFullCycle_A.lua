-- scenarioTroopsFullCycle_A.lua
-- Self-contained 8-step troop cycle test via Witchcraft.
-- Single injection — no delays — full trace in DCS.log via env.info().
--
-- Pre-requisites:
...
--
-- HOW TO USE: inject once → full 8-step result in DCS.log
...
local function report(msg)
    trigger.action.outText("[TFC_A] " .. msg, 30)
    log(msg)
end
...
local function fail(msg)
    trigger.action.outText("[TFC_A] !! FAIL: " .. msg, 60)
    log("FAIL: " .. msg)
    error("TFC_A_ABORT")
end

-- ══════════════════════════════════════════════════════════════════════════════
-- PLAYER RESOLUTION (with retry — Witchcraft may fire before player fully in)
-- ══════════════════════════════════════════════════════════════════════════════
...
for attempt = 1, 10 do
    ...
    if #units > 0 and units[1]:isExist() then
        ...
        env.info("[TFC_A] player resolved on attempt " .. attempt .. ": " .. playerName)
        break
    end
    if attempt < 10 then
        local t = os.clock() + 1
...
if not playerUnit or not playerUnit:isExist() then
    error("ABORT: no BLUE player — occupy a slot first")
end
...
-- ══════════════════════════════════════════════════════════════════════════
log("═══════════════════════════════════════════════════════")
log("START scenarioTroopsFullCycle_A | player=" .. playerName)
log("═══════════════════════════════════════════════════════")
...
-- STEP 3: disembark 1 → spawn DCS group + register JTAC individually
log("─── STEP 3 ───")
...
local jtacBefore = 0
for _ in pairs(jtacMgr.jtacs) do jtacBefore = jtacBefore + 1 end
...
pass("Step5 — embarkFromField: JTAC×" .. #deregNames .. " deregistered, group destroyed (no spurious killJTAC)")
...
local jtacBeforeR = 0
for _ in pairs(jtacMgr.jtacs) do jtacBeforeR = jtacBeforeR + 1 end
...
log("Step6 OK: JTAC before=" .. jtacBeforeR .. " after=" .. jtacAfterR .. " state=" .. tostring(jtacEntry and jtacEntry.state))
...
local jtacBeforeR7 = 0
for _ in pairs(jtacMgr.jtacs) do jtacBeforeR7 = jtacBeforeR7 + 1 end
...
log("═══════════════════════════════════════════════════════")
log("Steps completed: 1-8")
log("Final — droppedGroups[BLUE]=" .. droppedBlue .. " | inTransit=" .. inTransit .. " | jtacs=" .. jtacFinal .. " | templates=" .. templates)
log("JTAC lifecycle: spawn×2 | onUnitDead→deregister | dereg before destroy | resume | return")
log("═══════════════════════════════════════════════════════")
...
cleanupAll()
log("All test objects removed. Scenario complete.")
report("Scenario complete — Full trace in DCS.log.")

log("═══════════════════════════════════════════════════════")
log("END scenarioTroopsFullCycle_A")
log("═══════════════════════════════════════════════════════")

return "scenarioTroopsFullCycle_A done"