...
-- End-to-end scenario: deploy one AA system per range category (short/mid/long) for BLUE and RED
--
-- BLUE systems (Batumi area, 2km east, 1km apart on X axis):
--   short : M1097 Avenger   (AA missile vehicle — standard multi-crate, single DCS unit)
--   mid   : NASAMS          (AA template: LN + Radar + CP)
--   long  : Patriot         (AA template: LN + Radar + ECS)
--
-- RED systems (Kobuleti area, 2km east, 1km apart on X axis):
--   short : 9K33 Osa        (AA missile vehicle — standard multi-crate, single DCS unit)
--   mid   : KUB             (AA template: LN + Radar)
--   long  : S-300           (AA template: TEL-C + TR + SR1 + SR2 + C2)
--
-- Two unpack flows:
--   "standard" : spawnCrate × N + unpackCrate × N + _spawnUnpacked (single-unit AA vehicle)
...
-- Each "aa" system spawns a dedicated proxy 100m SOUTH of its crate position (heading=0 → north)
-- so that origin lands exactly on the crates (within 500m search radius).
...
local function report(msg)
    trigger.action.outText("[AA] " .. msg, 30)
    ctld.utils.log("INFO", "[scenario_aa_systems] " .. msg)
end
...
-- Destroy CTLD_UNP_* AA vehicles and proxy units from previous runs
...
-- Proxies are spawned 100m SOUTH of their system (heading=0 → origin lands on crates).
-- 3s gap before _doUnpack gives DCS time to register all statics and proxies.
...
for idx, sys in ipairs(SYSTEMS) do
    ...
    for i, w in ipairs(sys.weights) do
        ...
        if not desc then
            return string.format("ABORT: no descriptor weight=%.2f for %s", w, sys.label)
        end
        ...
        if not crate then
            return string.format("ABORT: spawnCrate failed weight=%.2f %s", w, sys.label)
        end
    ...
    -- Per-system proxy for "aa" flow: 100m south, heading north (0 rad)
    -- → _computeOrigin adds 100m north → origin = (sys.cx, sys.cz) = AT crates
    local proxyName = nil
    if sys.flow == "aa" then
...
local function _doUnpack(sys)
    -- Standard flow: single-unit AA vehicle (Avenger, Osa)
    ...
    -- AA template flow: per-system proxy 100m south, origin = AT crates
    ...
    -- Cleanup proxy immediately after use
...
return string.format("scenario_aa_systems started | 6 systems | BLUE@(%.0f,%.0f) RED@(%.0f,%.0f)",
    BX0, BZ, RX0, RZ)
