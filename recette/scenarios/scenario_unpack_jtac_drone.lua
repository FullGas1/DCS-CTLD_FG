...
-- End-to-end scenario: MQ-9 JTAC drone full lifecycle via crate spawn+unpack
--
...
--
-- Timings (all from T=0 injection):
--   T+0s   : (0) cleanup all RED vehicles + all flying JTACs
--             (1) spawn MQ-9 crate 5m in front of uh1-1
--             (2) unpack crate → MQ-9 deployed → startLase via _dispatchPostSpawn
--   T+5s   : draw BLUE orbit circle (r=2000m, initial route)
--   T+120s : VERIFY 1 — drone on initial orbit, state=idle (no target yet)
--             (3) spawn RED non-armed target 5km south of Batumi
--   T+150s : VERIFY 2 — drone lasing target (currentTarget set)
--             draw RED circle (r=1000m, target orbit)
--   T+330s : (4) destroy RED target
--   T+345s : VERIFY 3 — target lost, drone returns to initial orbit
--   T+645s : VERIFY 4 — drone still alive on initial orbit
--             (5) cleanup drone
...
local CRATE_OFFSET = 5        -- m in front of helo

-- RED target spawn: 5km south of Batumi (x=-356437-5000=-361437), on land (h≈680m)
local TARGET_GRP   = "JTAC_TEST_RED_TARGET"
local TARGET_UNIT  = "JTAC_TEST_RED_TARGET-1"
local TARGET_TYPE  = "Truck_URAL_4320_Cab"   -- unarmed, lockMode "vehicle" compatible
...
local function report(msg)
    trigger.action.outText("[DRONE] " .. msg, 30)
    ctld.utils.log("INFO", "[scenario_unpack_jtac_drone] " .. msg)
end
...
-- Destroy all flying JTACs
local toKill = {}
for gname, jtac in pairs(jmgr.jtacs) do table.insert(toKill, gname) end
...
-- Destroy all CTLD_UNP_* and CTLD_AIR_* groups
...
if not heloUnit or not heloUnit:isExist() then
    return "ABORT: helo unit not found: " .. HELO_NAME
end

local desc = cmgr:findDescriptorByWeight(MQ9_WEIGHT)
if not desc then return "ABORT: MQ-9 descriptor not found (weight=" .. MQ9_WEIGHT .. ")" end
...
if not crate then return "ABORT: spawnCrate failed for MQ-9" end
...
-- Mirrors menu flow: unpackCrate (state) + _spawnUnpacked (DCS spawn) separately
-- spawnPos = 50m in front of helo (same logic as menu safeDist)
...
if not spawnPos then
    return "ABORT: getSpawnObjectPositions failed"
end
...
-- ── T+480s: destroy RED target (5min after spawn to observe lasing) ──────────
...
-- ── T+495s: VERIFY 3 — target lost, drone returning ──────────────────────────
...
return string.format("scenario_unpack_jtac_drone started | crate='%s' | helo='%s' @ (%.0f,%.0f)",
