---@diagnostic disable
-- CTLD_troopManager.lua
-- Troop transport: load (PKZ), deploy (fast-rope / combat drop), extract, return-to-base (PKZ).
--
-- At init, generates one GROUND descriptor per loadable template in CTLDObjectsDescDb._db,
-- so every troop spawn goes through the unified CTLDObjectsDescDb.spawnObject() path.
-- Descriptors use formation = { type = "circle" }: unit offsets are computed at spawn time
-- using circleRadius = getSecureDistanceFromUnit + ctld.gs("spawnDistanceInCircle").
--
-- "Unload / Extract Troops" button logic (mirrors original):
--   1. On ground + nearest friendly dropped group + no troops onboard → extract()
--   2. Otherwise, if troops onboard:
--        - In PKZ zone → returnToBase()   (troops returned to zone pool)
--        - Elsewhere  → deploy()          (fast-rope or combat drop; EXZ counted, no spawn)
--
...
function CTLDTroopManager:init()
    self._inTransit     = {}              -- [unitName] = transit data (see loadFromZone)
...
-- Returns the in-transit data for unitName, or nil.
...
function CTLDTroopManager:getWeight(unitName)
    local t = self._inTransit[unitName]
    return t and t.weight or 0
end
...
function CTLDTroopManager:deploy(unit)
    local unitName = unit:getName()
    local transit  = self._inTransit[unitName]
    ...
    if exzZone then
        ...
        trigger.action.setUserFlag(exzZone.flagName, current + transit.unitTotal)
        ctld.utils.log("INFO", "deploy: %d troops sent to EXZ '%s' (flag %s = %d)",
            transit.unitTotal, exzZone.zoneName, exzZone.flagName, current + transit.unitTotal)
    else
        ...
        local group = CTLDObjectsDescDb.spawnObject(
            transit.templateKey,
            transit.coalitionId,
            transit.countryId,
            pt.x, pt.z, hdg,
        ...
        if not group then
            ctld.utils.log("ERROR", "deploy: spawnObject failed for key '%s'", transit.templateKey)
            return false
        end

        table.insert(self._droppedGroups[transit.coalitionId], group:getName())

        if transit.hasJtac then
            -- Signal: CTLDJtacManager will handle laser attribution when built
            ctld.utils.log("INFO", "deploy: JTAC group dropped — '%s'", group:getName())
        end
    end
    ...
    trigger.action.outTextForCoalition(transit.coalitionId,
        ctld.tr("%1 %2 [%3] from %4 %5.",
            self:_callsign(unit), method, transit.templateName, unit:getTypeName(), dest), 10)
...
function CTLDTroopManager:returnToBase(unit, zone)
    ...
    if zone.limit >= 0 then
        zone.limit = zone.limit + transit.unitTotal
        if zone.flagName then
            trigger.action.setUserFlag(zone.flagName, zone.limit)
        end
    end
    ...
    ctld.utils.log("INFO", "returnToBase: '%s' returned [%s] to PKZ '%s'",
        unitName, transit.templateName, zone.zoneName)
...
function CTLDTroopManager:_menuCheckCargo(unit)
    ...
    if t then
        msg = ctld.tr("Cargo: [%1] — %2 troops, %3 kg",
            t.templateName, t.unitTotal, math.floor(t.weight))
    else
